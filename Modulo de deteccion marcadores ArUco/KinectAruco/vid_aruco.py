...
#Get frame and convert to grayscale
    ...
    #Si identifica algo:
    if ids is not None:
        #Se elimino del programa la parte de dibujar sobre la imagen los datos sino que solo imprime el número del aruco que detectó
        rvec,tvec, _objPoints= aruco.estimatePoseSingleMarkers(corners, marker_size, camera_matrix, camera_distortion)
        for marker in range(len(ids)):
            print(ids[marker][0],rvec[marker],tvec[marker])
